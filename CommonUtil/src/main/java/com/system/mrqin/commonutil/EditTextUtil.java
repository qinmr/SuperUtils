...
public class EditTextUtil {
        ...
        switch (type) {
            case TYPE_VERIFY:
                if (type == TYPE_VERIFY && inputed.length() < 4) {
                    return showInputedError(context, et, StringUtil.isNotEmpty(errorRemind) ? errorRemind : "验证码不能小于4位");
                }
                break;
            case TYPE_PASSWORD:
                if (inputed.length() < 6) {
                    return showInputedError(context, et, StringUtil.isNotEmpty(errorRemind) ? errorRemind : "密码不能小于6位");
                }
                if (!StringUtil.isNumberOrAlpha(inputed)) {
                    return showInputedError(context, et, StringUtil.isNotEmpty(errorRemind) ? errorRemind : "密码只能含有字母或数字");
                }
                break;
            case TYPE_PHONE:
                if (inputed.length() != 11) {
                    return showInputedError(context, et, StringUtil.isNotEmpty(errorRemind) ? errorRemind : "请输入11位手机号");
                }
            ...
            case TYPE_MAIL:
                ...
                break;
            default:
                if (!StringUtil.isNotEmpty(inputed) || inputed.equals(StringUtil.getTrimedString(et.getHint()))) {
                    return showInputedError(context, et, StringUtil.isNotEmpty(errorRemind) ? errorRemind : StringUtil.getTrimedString(et));
                }
                break;
        }
    ...
    public static boolean showInputedError(Activity context, EditText et, String string) {
        if (context == null || !StringUtil.isNotEmpty(string)) {
